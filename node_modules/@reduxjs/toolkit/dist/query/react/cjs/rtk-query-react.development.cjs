...
__reExport(src_exports, require("redux"));
...
var import_utils17 = require("@standard-schema/utils");
var NamedSchemaError = class extends import_utils17.SchemaError {
...
function buildThunks({
  ...
  const updateQueryData = (endpointName, arg, updateRecipe, updateProvided = true) => (dispatch, getState) => {
    ...
    if ("data" in currentState) {
      if ((0, import_immer7.isDraftable)(currentState.data)) {
        const [value, patches, inversePatches] = (0, import_immer7.produceWithPatches)(currentState.data, updateRecipe);
    ...
    updateQuerySubstateIfExists(draft, meta.arg.queryCacheKey, (substate) => {
      ...
      if (merge) {
        if (substate.data !== void 0) {
          ...
          let newData = (0, import_immer6.produce)(substate.data, (draftSubstateData) => {
        ...
        substate.data = definitions[meta.arg.endpointName].structuralSharing ?? true ? copyWithStructuralSharing((0, import_immer8.isDraft)(substate.data) ? (0, import_immer9.original)(substate.data) : substate.data, payload) : payload;
      }
      ...
      queryResultPatched: {
        reducer(draft, {
          ...
          updateQuerySubstateIfExists(draft, queryCacheKey, (substate) => {
            substate.data = (0, import_immer9.applyPatches)(substate.data, patches.concat());
          });
        },
        prepare: prepareAutoBatched()
      }
    ...
    reducers: {
      subscriptionsUpdated: {
        reducer(state, action) {
          return (0, import_immer9.applyPatches)(state, action.payload);
        },
        prepare: prepareAutoBatched()
      }
    }
...
var defaultQuerySubState = /* @__PURE__ */ (0, import_immer6.produce)(initialSubState, () => {
});
var defaultMutationSubState = /* @__PURE__ */ (0, import_immer6.produce)(initialSubState, () => {
});
...
var import_reselect3 = require("reselect");
function buildCreateApi(...modules) {
  return function baseCreateApi(options) {
    const extractRehydrationInfo = (0, import_reselect3.weakMapMemoize)((action) => options.extractRehydrationInfo?.(action, {
    ...
    const context = {
      ...
      apiUid: nanoid(),
      extractRehydrationInfo,
      hasRehydrationInfo: (0, import_reselect3.weakMapMemoize)((action) => extractRehydrationInfo(action) != null)
    };
    const api = {
      injectEndpoints,
      enhanceEndpoints({
      ...
      for (const [endpointName, definition] of Object.entries(evaluatedEndpoints)) {
        if (inject.overrideExisting !== true && endpointName in context.endpointDefinitions) {
          if (inject.overrideExisting === "throw") {
            throw new Error(false ? _formatProdErrorMessage(39) : `called \`injectEndpoints\` to override already-existing endpointName ${endpointName} without specifying \`overrideExisting: true\``);
        ...
        if (typeof process !== "undefined" && true) {
          if (isInfiniteQueryDefinition(definition)) {
            ...
            if (typeof maxPages === "number") {
              if (maxPages < 1) {
                throw new Error(false ? _formatProdErrorMessage2(40) : `maxPages for endpoint '${endpointName}' must be a number greater than 0`);
              }
...
function fakeBaseQuery() {
  return function() {
    throw new Error(false ? _formatProdErrorMessage(33) : "When using `fakeBaseQuery`, all queries & mutations must use the `queryFn` definition syntax.");
  };
}
    ...
    if (didMutate) {
      if (!updateSyncTimer) {
        updateSyncTimer = setTimeout(() => {
          ...
          const [, patches] = (0, import_immer10.produceWithPatches)(previousSubscriptions, () => newSubscriptions);
...
var coreModule = ({
  createSelector: createSelector2 = import_reselect2.createSelector
} = {}) => ({
  name: coreModuleName,
  init(api, {
    baseQuery,
    ...
    (0, import_immer11.enablePatches)();
...
// src/query/react/module.ts
var import_react_redux3 = require("react-redux");
var import_reselect4 = require("reselect");
...
=======
  createSelector: createSelector2 = import_reselect4.createSelector,
>>>>>>> efaa580b454fae0aebd62b00175b0703e383c994
...
=======
        throw new Error(false ? _formatProdErrorMessage(36) : `When using custom hooks for context, all ${hookNames.length} hooks need to be provided: ${hookNames.join(", ")}.
>>>>>>> efaa580b454fae0aebd62b00175b0703e383c994
...
=======
    throw new Error(false ? _formatProdErrorMessage(35) : "Existing Redux context detected. If you already have a store set up, please use the traditional Redux setup.");
  }
