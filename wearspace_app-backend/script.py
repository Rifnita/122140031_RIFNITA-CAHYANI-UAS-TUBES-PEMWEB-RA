...
def extract_text_to_markdown(base_dir, output_file="context.md"):
    ...
    target_directories = [
        os.path.join(base_dir, "wearspace_app", "alembic"),
        os.path.join(base_dir, "wearspace_app", "models"),
        os.path.join(base_dir, "wearspace_app", "scripts"),
        os.path.join(base_dir, "wearspace_app", "views"),
        os.path.join(base_dir, "wearspace_app")
    ]

    apcer_root = os.path.join(base_dir, "wearspace_app")
    if not os.path.isdir(apcer_root):
        print(f"Error: Direktori '{apcer_root}' tidak ditemukan.")
        return
    ...
    with open(output_path, 'w', encoding='utf-8') as outfile:
        ...
        print(f"Memproses file-file langsung di direktori: {apcer_root}")
        for item in os.listdir(apcer_root):
            item_path = os.path.join(apcer_root, item)
